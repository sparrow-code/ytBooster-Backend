  ...
  return new Promise(async (resolve) => {
    try {
      if (
        ...
        !url.match(/(https|http):\/\/www.instagram.com\/(p|reels|tv|stories)/gi)
      )
        return resolve({
          status: false,
      ...
      if ($("table.table").length || $("article.media > figure").length) {
        ...
        $("div.download-items").each((_, elem) => {
          const isVideo =
            $(elem).find("div.download-items__thumb.video").length > 0;
          const thumbnail = $(elem).find("img").attr("src");
          const urlElem = $(elem).find("a");
          const url = urlElem.attr("href");

          if (isVideo && thumbnail && url) {
            results.video.push({ thumbnail, url });
          } else if (thumbnail && url) {
            results.photo.push(url);
          }
        });
      }
